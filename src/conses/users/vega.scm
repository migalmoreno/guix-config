...
(define extra-init-el
  '((add-hook 'after-init-hook 'server-start)
    ...
    (defun rde-linux-list-lsusb-devices ()
      "List recognized lsusb devices."
      (interactive)
      (completing-read
       "Device: "
       (with-temp-buffer
         (call-process ,(file-append
                         (@ (gnu packages linux) usbutils)
                         "/bin/lsusb")
                       nil t)
         (goto-char (point-min))
    ...
    (defun rde-linux-toggle-lsusb-device ()
      "Toggle active status of lsusb device."
      (interactive)
      (let* ((device (rde-xorg-list-lsusb-devices))
             (bus (with-temp-buffer
                    (call-process ,(file-append
                                    (@ (gnu packages linux) usbutils)
                                    "/bin/lsusb")
                                  nil t)
              ...
              (with-temp-buffer
                (let (value)
                  (dolist (mapping devices value)
                    (let ((dir (format "/sys/bus/usb/devices/%s" mapping)))
                      (when (file-exists-p (expand-file-name "idProduct" dir))
                        (insert-file-contents
                         (expand-file-name "idProduct" dir)
                         nil nil nil t)
                        (when (string= (substring (buffer-string) 0 -1) product-id)
                          (setq value mapping)))))
                  value)))
             (is-enabled
              (with-temp-buffer
            ...
            (progn
              (write-region
               ...
               (format "/sudo::/sys/bus/usb/devices/%s/bConfigurationValue"
                       device-mapping))
              (message (format "Device %S disabled" device)))
          (write-region
           "1" nil
           (format "/sudo::/sys/bus/usb/devices/%s/bConfigurationValue"
                   device-mapping))
