(define-module (contrib features xorg)
  ...
  #:use-module (rde home services xorg)
     ...
     (rde-elisp-configuration-service
      ...
      `((require 'hexrgb)
        ...
        (defun rde-xorg-take-screenshot (&optional region)
          ...
          (when-let* ((pictures-dir (xdg-user-dir "PICTURES"))
                      (file-name (replace-regexp-in-string
                                  (rx space) "0"
                                  (format-time-string "%Y%m%e-%H%M%S.jpg")))
        ...
        (defun rde-xorg--toggle-xinput-device (device)
          "Toggle active status of xinput DEVICE, such as 'Touchpad'."
          (let* ((xinput-bin ,(file-append xinput "/bin/xinput"))
                 (id (with-temp-buffer
                       (call-process xinput-bin)
                       (goto-char (point-min))
                       (re-search-forward
                        (rx (* any) (literal device) (* any)
                            "id=" (group (+ num)) space (* any))
                        nil t)
                       (match-string 1)))
                 (is-enabled (with-temp-buffer
                               (call-process xinput-bin nil nil nil
                                             "list-props" id)
                               (goto-char (point-min))
                               (re-search-forward
                                (rx (* any) "Device Enabled (186):"
                                    (+ space) (group num) (* any))
                                nil t)
                               (match-string 1))))
        ...
        (defun rde-xorg--toggle-lsusb-device (device)
          ...
          (let* ((bus (with-temp-buffer
                 ...
                 (devices (directory-files "/sys/bus/usb/devices" nil
                                           (rx (* any) num "-" num eol)))
                 (device-mapping
                  (with-temp-buffer
                    (let (value)
                      (dolist (mapping devices value)
                        (insert-file-contents
                         (format "/sys/bus/usb/devices/%s/idProduct" mapping)
                         nil nil nil t)
                        (when (equal (substring (buffer-string) 0 -1)
                                     product-id)
                          (setq value mapping)))
                      value)))
                 (is-enabled
                  (with-temp-buffer
                    (insert-file-contents
                     (format
                      "/sys/bus/usb/devices/%s/bConfigurationValue"
                      device-mapping))
                    (substring (buffer-string) 0 -1))))
            (if (string-equal is-enabled "1")
                (write-region
                 "0" nil
                 (format "/sudo::/sys/bus/usb/devices/%s/bConfigurationValue"
                         device-mapping))
                (write-region
                 "1" nil
                 (format "/sudo::/sys/bus/usb/devices/%s/bConfigurationValue"
      ...
      #:commentary "Provide various helper functions for useful Xorg-related \
operations.")
     (simple-service
